...
//   logo: "eventLogo.png",
//   about:
//     "Design Mania 2025 presented by Codigo Plataforma is a team(maximum of 3 members) UI/UX design competition where participants will demonstrate their research, creativity, and design skills. Participants will receive a case study, and they will have 24 hours to conduct research. Once research is submitted, participants will then design according to the case study.",
...
export default function Page() {
  ...
  useEffect(() => {
    ...
    async function loadTeamForUser(u) {
      ...
      const sic =
        u.sic || u.sic_no || u.sicNo || u.SIC || u.sicNumber || u.sic_number || u.id;
      if (!sic) {
        if (mounted) {
          setTeamInfo(null);
          setAlreadyRegistered(false);
  ...
  return (
    <main className="min-h-screen bg-gray-50 text-slate-800">
      {/* Hero */}
      <section className="relative">
        <div className="h-48 sm:h-64 md:h-96 w-full overflow-hidden relative">
          {/* eslint-disable-next-line @next/next/no-img-element */}
          <img
          ...
          />
          {/* Keep it BRIGHT — very light wash, no dark overlay */}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6 md:gap-8">
          <main className="lg:col-span-2 space-y-4 sm:space-y-6 md:space-y-8">
            ...
            <article className="bg-white border rounded-lg p-4 sm:p-5 md:p-6 shadow-sm">
              <h3 className="text-base sm:text-lg font-semibold">Eligibility</h3>
              <p className="mt-2 sm:mt-3 text-sm sm:text-base text-slate-700 leading-relaxed">
                This event is open to all the students of Silicon Institute of Technology,
                Sambalpur to showcase their UI/UX skills. Teams (up to maximum 3 members) are
                welcome. Participants must register before {demoEvent.importantDate} and agree to
          ...
          <aside className="space-y-4 sm:space-y-6">
            <div className="lg:sticky lg:top-24 space-y-3 sm:space-y-4">
              <div className="bg-white border rounded-lg p-3 sm:p-4 shadow-sm">
                ...
                <div className="mt-3 text-xs sm:text-sm text-slate-700 space-y-1">
                  <div>
                    Total registered: <span className="font-semibold">{regCount}</span>
                  </div>
                  <div>
                    Team size:{" "}
                    <span className="font-semibold">{teamSize ? `${teamSize}` : "3"}</span>
                  </div>
                </div>
              </div>
